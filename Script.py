...
📙<b> 📙 Команды:
	...
	+ Введите: .r <эмодзи>
	📂 Скрипт анимации Фашизма ".яфашист 1-5"</b> \n
...
@app.on_message(filters.command("чтозахуйня", prefixes=".") & filters.me)
def mems(app, msg):
	msg.delete()
...
''')
	...
	sleep(0.1)
	app.send_message(message.chat.id, f'''
	 <b>ты понимаешь что ты такой потасканный пидарас после моего хуя ,что ты даже ходить уже нормально не можешь после моего хуя, ты просто валишься как шкаф ебаный сука</b>
	 ''')
	sleep(0.1)
	app.send_message(message.chat.id, f'''
	 <b>ты же блять на моем хую живешь и еще пытаешься выебыватся чтоле, я тебе щас лещей хуем накидаю, пес ты сутулый</b>
	 ''')
	sleep(0.1)
	app.send_message(message.chat.id, f'''
	 <b>хуеглот ебучий) сын путаны раздолья) подсос моего хуя) суда ползи спидозник) ты хули умолк хуй сосёшь не многоли чести</b>
	 ''')
	sleep(0.1)
	app.send_message(message.chat.id, f'''
	 <b>пойми) мой хуй ща будет ездить по твоей голове как гозоно косилка по траве</b>
	 ''')
	sleep(0.1)
	app.send_message(message.chat.id, f'''
	 <b>эй крыса концелярская я тебе щас карсар в жопу суну и подожгу) едобир ахуевший</b>
	 ''')
	sleep(0.1)
	app.send_message(message.chat.id, f'''
	 <b>ты понимаешь что я ее щас хуем буду как мяч футбольный пинать и в твою пиду ее закачу а еще твоя мать любит мой хуй получать в качестве подарка на новый год потом сосёт с 31 по 15 не отходя от моего леденца блядь</b>
	 ''')
	sleep(0.1)
	app.send_message(message.chat.id, f'''
	 <b>ты понимаешь что мой хуй еще за год вперёд расчитался с пиздой твоей мамашки, ведь её так морозит когда мой хуй проводит по её пизде словно муражки по коже</b>
	 ''')
	sleep(0.1)
	app.send_message(message.chat.id, f'''
	 <b>ты понимаешь что я твоей матери сейчас хуем буду делать ампутацию матки </b>
	 ''')
	sleep(0.1)
	app.send_message(message.chat.id, f'''
	 <b>ты понимаешь что ты ебаный шизофренник порожденный злом моего хуя</b>
	 ''')
